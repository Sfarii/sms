...
namespace SMS\StudyPlanBundle\BaseController;
...
class BaseController extends Controller
{
    ...
    {
        if (!$this->has('sms.study_plan.entity_manager')){
           throw $this->createNotFoundException('Service Not Found');
        }
        return $this->get('sms.study_plan.entity_manager');
    }
	    ...
	    if ($form->count() > 0) {
	        foreach ($form->all() as $child) {
						foreach ($form[$child->getName()]->getErrors() as $error) {
							$errors[sprintf("%s",$child->getName())] = $error->getMessage();
						}
	        }
	    }
    	return $errors;
