...
/**
 * Class EstablishmentDatatable
 *
 * @package SMS\SchoolBundle\Datatables
 */
class EstablishmentDatatable extends AbstractDatatableView
{
    /**
     * {@inheritdoc}
     */
    public function buildDatatable(array $options = array())
    {
        ...
        $this->ajax->set(array(
            'url' => $this->router->generate('establishment_results'),
            'type' => 'GET',
            'pipeline' => 0
        ));
        ...
        $this->columnBuilder
            ->add(null, 'multiselect', array(
                'actions' => array(
                    array(
                        'route' => 'establishment_bulk_delete',
                        'icon' => '&#xE872;',
            ...
            ->add('establishmentName', 'column', array(
                'title' => $this->translator->trans('establishment.field.establishmentName'),
            ))
            ->add('phone', 'column', array(
                'title' => $this->translator->trans('establishment.field.phone'),
            ))
            ->add('email', 'column', array(
                'title' => $this->translator->trans('establishment.field.email'),
            ))
            ->add('address', 'column', array(
                'title' => $this->translator->trans('establishment.field.address'),
            ))
            ->add(null, 'action', array(
                'title' => $this->translator->trans('datatables.actions.title'),
                'actions' => array(
                    array(
                        'route' => 'establishment_show',
    ...
    {
        return 'establishment_datatable';
    }
}
