...
class ResettingController extends BaseController
{
     ...
     * @ParamConverter("user", class="SNSBundlesUserBundle:BaseUser", options={
     ...
     *
     */
    public function resettingPasswordAction(BaseUser $user, Request $request)
    {
        // registration form 
        $form = $this->createForm(ResettingPasswordFormType::class)->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $this->getUserEntityManager()->resettingNewPassword($user , $form->get('plainPassword')->getData() , $this->getParameter("tokenLifetime"));
            return $this->render(
                    'smsuserbundle/user/resetting/index.html.twig',
                    array(
                        "msg" => $this->get('translator')->trans('reset.success')
                    )
                );
        }
